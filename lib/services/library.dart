...
class LibraryService extends ChangeNotifier {
  ...
  Future<String> createPlaylistKey(String title, {Map? item}) async {
    if (title.trim().isEmpty) {
      throw Exception("Playlist name can't be empty");
    }
    String key = "CSTMPL${DateTime.now().millisecondsSinceEpoch}";
    await _box.put(
      key,
      {
        'title': title,
